...
public class PlaywithPointers {
	...
	public CharPtr recurseReversal(CharPtr node, CharPtr end) {
		if (node == null) {
			return null;
		}

		if (node.charObj == null) {
			return null;
		}
		if (node != null && node.charObj != null) {
			CharPtr next = node.charObj;
			node.charObj = end;
			return null != next ? recurseReversal(next, node) : node;
		}
		return null;
	}
}
